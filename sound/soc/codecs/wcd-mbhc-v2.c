...
#ifdef CONFIG_LGE_MOISTURE_DETECTION_ELEC_IRQ
...
static bool lge_moisture_detection(struct wcd_mbhc *mbhc, int switch_device)
{
	...
	if (!strcmp(mbhc->sdev.name, LGE_SWITCH_NAME_AUX) && (switch_device == LGE_HEADSET)) { // 4pole aux case
		if ((mbhc->zl_org > MOISTURE_Z_MIN && mbhc->zl_org < MOISTURE_Z_MAX)
			...
			result = true;
		else if (mbhc->zr_org == TASHA_ZDET_FLOATING_IMPEDANCE) // 1.both L and R float case,  2. L: 0 R:Float,  3. L:not-zero R:Float
			result = true;
			...
			result = true;
		else if (mbhc->zl_org == 0 && mbhc->zr_org > 600) // L : 0 , R > 600
			result = true;
		else if (mbhc->zl_org != 0 && mbhc->zr_org == 0) // L : not-zero  R : 0
			result = true;
		else if (mbhc->zl_org == 0 && mbhc->zr_org == 0) // L : 0  R : 0
			result = true;
		else
			result = false;
	}
...
static irqreturn_t wcd_mbhc_hs_ins_irq(int irq, void *data)
{
	...
	pr_info("[LGE MBHC] %s: enter\n", __func__);
	pr_debug("%s: enter\n", __func__);
#ifndef CONFIG_LGE_MOISTURE_DETECTION_ELEC_IRQ
	if (!mbhc->mbhc_cfg->detect_extn_cable) {
	...
	if (detection_type) {
#if defined(CONFIG_LGE_MOISTURE_DETECTION) && defined(CONFIG_LGE_MOISTURE_DETECTION_ELEC_IRQ)
